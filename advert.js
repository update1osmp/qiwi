...
(
    null,
    {
        ...
        actionScript: function(fileName, funcParam, isDelete) {
            ...
            if ($(funcParam).innerHTML == '')
                this.defaultBanner(funcParam);
        },
        
        defaultBanner: function(place) {
            if ((this.pageName == 'Main' || this.pageName == 'Pages' ||
                this.pageName == 'Adv_Cellular' || this.pageName == 'Provider' ||
                this.pageName == 'Validate_Confirm' || this.pageName == 'qiwi_promo' ||
                this.pageName == 'epa' || this.pageName == 'Bank_Page') && place == 'jsPlaceHere_1') {
                $('advert_1').innerHTML = getFlashDef("./swf/banner.swf", false);
            }
        },
        ...
        getGroupId: function() {
            var aResult = "";
            var sGrpId = storage.get("groupId");
            if (!$isNoU(sGrpId) && sGrpId != "null") {
                storage.remove("groupId");
                aResult = sGrpId.split("|");
            }
            return aResult;
        },
        ...
        setAdvertItemList: function() {
            ...
            if (this.advertPlaceList.length > 0) {
                ...
                for (var i = 0; i < this.advertPlaceList.length; i++) {
                    ...
                    contentArr = this.getProviderContent(advPlaceItem, this.providerId, this.aGroupId);
        ...
        getProviderContent: function(name, provider, group) {
            var result = "";
            ...
            for (var i = 0; i < contentCount; i++) {
                ...
                if (group.length != 0) {
                    for (var j = group.length - 1; j >= 0; j--) {
                        if (this.isProviderInArray(group[j], result[2])) {
                            bGrp = true;
                            break;
                        }
                    }
            ...
            switch (contentType) {
                ...
                default:
                    if (isFirstPlace) {
                        switch (this.pageName) {
                            case "Main" :
                                html = getFlashDef("./swf/banner_main.swf", true);
                                break;
                            case "Pages" :
                                html = getFlashDef("./swf/banner_pages.swf", true);
                                break;
                            case "Adv_Cellular" :
...
function setimage(place, image, div, content_td, height) {
    ...
    switch (contentType) {
        ...
        case 'script':
            insContent =
	          '<table cellpadding="0" cellspacing="0" border="0" width="100%" height="100%" bgcolor="#ff7800">' +
	          '<tr><td align="center" valign="middle" class="on_page_title"><div id="jsPlaceHere"></div></td></tr></table>';
            break;
